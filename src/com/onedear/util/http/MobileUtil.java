...
public class MobileUtil {
	private static String isMobileRegex1 = ".*(android.+mobile|ucweb|avantgo|bada\\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od|ad)|iris|kindle|lge |maemo|midp|mmp|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\\/|plucker|pocket|psp|symbian|treo|up\\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino).*";
	...
	//手机品牌型号
	private static String mobileModleRegex = ".*((nokia)[^\\s/;_]*)|;([\\s\\S&&[^;]]*)\\sbuild|((sonyEricsson|samsung|htc|lenovo|blackberry|rim|lg|gt-|oppo|philips|desire)\\s?[^\\s\\/;]*)|((huawei|mot)\\s?[^\\s\\/;_]*)|(ipad)|(iphone)|(ipod)|(ios)|(meizu[^\\s;,|]*)";
	private static Pattern mobileModleRegexPattern = Pattern.compile(mobileModleRegex,Pattern.CASE_INSENSITIVE);
	//手机系统
	private static String mobileOsRegex = "(symbianOS/\\S*)|(series60)|(series40)|(symbian/\\S*)|(android\\s[^\\s;]*)[;|\\s]|(iphone\\sos\\s?\\S*)|(ios\\s[\\S&&[^;]]*)|(cpu\\sos\\s[\\S&&[^;]]*)|(windows\\s(phone|ce|mobile)\\s?[[^\\s);,]]*)"; 
	private static Pattern mobileOsRegexPattern = Pattern.compile(mobileOsRegex,Pattern.CASE_INSENSITIVE);
	
	
	private static String mobileBrowserUcs = "(ucweb[\\S&&[^/]]*)";
	...
	public static boolean isMobile(String userAgent) {
		...
		if(userAgent.matches(isMobileRegex1) || userAgent.substring(0,4).matches(isMobileRegex2))
			return true ;
		return false ;
	}	
	...
	public static String getMobileModle(String userAgent ) {
		if(userAgent == null)
			return null ; 
		userAgent = userAgent.toLowerCase();
	...
	private static String verifyMobile(String mobile) {
		...
		if(mobile.indexOf("sonyericsson")!= -1)
			mobile = mobile.replace("sonyericsson", "SonyEricsson");
		if(mobile.indexOf("rim") !=-1)
			mobile = mobile.replace("rim", "blackberry");
		return mobile;
	}
	...
	public static void main(String[] args) {
		...
		String sonyEricsson3 = "SonyEricssonK750i/R1CA/SN356554009124815 Java/SEMC-Java/2.0";
		...
		String samsung2 = "SAMSUNG-GT-S5520/1.0 SHP/VP";
		...
		String iphone2 = "Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_5 like Mac OS X; zh-cn) Mobile/8L1 Safari/6533.18.5";
		...
		String htc = "JUC (Linux; U; 2.3.3; zh-cn; HTC_Incredible_S 23; 480*800)";
		...
		String mobile = getMobileModle(userAgent);
